...
import { messageToStatusCode } from '../utilities/message-to-status-code.utility';
...
export class AllExceptionsFilter extends BaseExceptionFilter {
  private readonly logger = new Logger(AllExceptionsFilter.name);
    ...
    const ignoreRoute = [
      '/auth/login',
      '/merchants/edit-password',
      '/auth/get-security',
      '/merchants/change-password',
    ...
    if (exception instanceof HttpException) {
      ...
      if (msg.includes('UNKNOWN:')) {
        // GRPC exception
        const arr = msg.split('UNKNOWN:');
        if (arr?.length > 0) {
          msg = arr[1].trim();
          // Check if error is `insufficient funds for gas`
          if (msg.toLowerCase().includes('insufficient funds for gas')) {
            messageCode = 'BLOCKCHAIN.INSUFFICIENT_FUNDS_FOR_GAS';
            description = CustomErrorMessage[messageCode].description;
    ...
    if (!messageCode) {
      if (msg && CustomErrorMessage.hasOwnProperty(msg)) {
        ...
        description = msg || UNKNOWN_ERR_DESC;
      }
    }
    // Response
    const responseObj = {
